function [bestsol,bestfitness,BestFitIter] = GeneticAlgorithm(prob,lb,ub,Np,T,etac,etam,Pc,Pm,B)
...
for p = 1:Np
    f(p) = prob(P(p,:),ub,lb,B);            % Evaluating the fitness function of the initial population
end
BestFitIter(1)=min(f);
...
for t = 1: T
    ...
    for j = 1:Np
        OffspringObj(j) = prob(offspring(j,:),ub,lb,B);     % Evaluating the fitness of the offspring solution
    end
